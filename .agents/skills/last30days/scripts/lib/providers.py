...
DASHSCOPE_DEFAULT = "qwen-plus"
DASHSCOPE_DEEP_RERANK = "qwen-max"
...
class DashScopeClient(ReasoningClient):
    ...
    def __init__(self, api_key: str, base_url: str | None = None):
        self.api_key = api_key
        self.base_url = (base_url or DASHSCOPE_CHAT_URL).rstrip("/")
...
_MODEL_DEFAULTS: dict[str, tuple[str, str]] = {
    ...
    "dashscope": (DASHSCOPE_DEFAULT, DASHSCOPE_DEFAULT),
}
...
def _resolve_model_pins(config: dict[str, Any], depth: str, provider_name: str) -> tuple[str, str, str]:
    ...
    if depth == "deep" and provider_name == "dashscope":
        default_rerank = DASHSCOPE_DEEP_RERANK
...
def resolve_runtime(config: dict[str, Any], depth: str) -> tuple[schema.ProviderRuntime, ReasoningClient | None]:
    ...
    dashscope_key = config.get("DASHSCOPE_API_KEY") or config.get("ALIBABA_API_KEY")
    ...
    if provider_name == "dashscope":
        if not dashscope_key:
            raise RuntimeError("DashScope selected but DASHSCOPE_API_KEY / ALIBABA_API_KEY is not configured.")
        base_url = config.get("DASHSCOPE_BASE_URL") or DASHSCOPE_CHAT_URL
        runtime = schema.ProviderRuntime(
        ...
        return runtime, DashScopeClient(dashscope_key, base_url)
