  ...
  return (
    <div className="h-screen w-screen flex overflow-hidden bg-[var(--background)]">
      ...
      <aside className="w-14 shrink-0 flex flex-col items-center py-3 gap-2 border-r border-[var(--border-subtle)] bg-[var(--card-bg)] z-20">
        ...
        <SidebarIcon icon={<FolderOpen className="w-4 h-4" />} active={activeTool === "files"} title="文件" onClick={() => setActiveTool("files")} />
        <SidebarIcon icon={<BarChart3 className="w-4 h-4" />} active={activeTool === "analytics"} title="分析" onClick={() => setActiveTool("analytics")} />
        <SidebarIcon icon={<Settings2 className="w-4 h-4" />} active={activeTool === "settings"} title="设置" onClick={() => setActiveTool("settings")} />
        ...
        <div className="absolute top-5 left-1/2 -translate-x-1/2 z-30">
          <div className="flex items-center gap-1 bg-[var(--card-bg)]/90 backdrop-blur border border-[var(--border-subtle)] rounded-full px-2 py-1.5 shadow-sm">
            ...
            <button
              ...
              title="适应画布"
            >
              <Maximize className="w-3.5 h-3.5" /> 适应
            </button>
            ...
            <button
              ...
              title="小地图"
            >
              <Compass className="w-3.5 h-3.5" /> 小地图
            </button>
          </div>
