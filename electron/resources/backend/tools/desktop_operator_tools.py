"""LangChain tools for Desktop Operator Agent."""
...
logger = setup_logger("desktop_operator_tools")
...
def run_desktop_automation(plan_json: str, working_dir: str) -> str:
    """Submit CLI automation plan for approval and execution. working_dir must be under My Computer roots."""
    try:
        plan = json.loads(plan_json)
    except json.JSONDecodeError as exc:
        return json.dumps({"success": False, "error": f"Invalid plan_json: {exc}"}, ensure_ascii=False)
    try:
        result = desktop_operator_service.submit_cli_execution(plan, working_dir)
        result["approval_url"] = "/settings/capabilities"
        return json.dumps(result, ensure_ascii=False, indent=2)
    except ValueError as exc:
        return json.dumps({"success": False, "error": str(exc)}, ensure_ascii=False)
...
@tool
def launch_desktop_app(app_id: str) -> str:
    """Launch a desktop application by id or name. Requires approval."""
    try:
        result = desktop_operator_service.launch_application(app_id)
        result["approval_url"] = "/settings/capabilities"
        return json.dumps(result, ensure_ascii=False, indent=2)
    except ValueError as exc:
        return json.dumps({"success": False, "error": str(exc)}, ensure_ascii=False)
...
def run_native_desktop_task(goal: str, app_hint: str = "") -> str:
    """Run native GUI automation (screenshot + click/type loop) for apps without CLI. Requires approval."""
    try:
        from services.native_use_service import start_native_use_task

        result = start_native_use_task(goal=goal, app_hint=app_hint)
        result["approval_url"] = "/settings/capabilities"
        return json.dumps(result, ensure_ascii=False, indent=2)
