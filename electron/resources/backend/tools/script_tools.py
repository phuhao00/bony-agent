...
def generate_script(
    ...
    Args:
        ...
        use_search: 是否搜索知识库获取上下文，默认为 True
    
    Returns:
        JSON格式的结构化脚本
    """
    logger.info(f"Generating script: topic={topic}, platform={platform}, duration={duration}s, use_search={use_search}")
    
    # RAG: 搜索知识库获取上下文
    rag_context = ""
    if use_search:
        try:
            logger.info(f"Searching knowledge base for topic: {topic}")
            search_result = search_knowledge_base.invoke(topic)
            if "Error" not in search_result and "no result" not in search_result.lower():
                 rag_context = f"\n\n## 知识库参考信息\n{search_result}"
                 logger.info("Found relevant context in knowledge base")
            else:
                 logger.info("No relevant context found in knowledge base or error occurred")
        except Exception as e:
            logger.warning(f"Failed to search knowledge base: {e}")

    # 合并补充信息
    final_additional_info = additional_info + rag_context
    ...
    try:
        ...
        response = chain.invoke({
            ...
            "style": style,
            "style_desc": style_desc,
            "additional_info": final_additional_info or "无"
        })
