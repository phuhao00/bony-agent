...
SEEDANCE_MAX_WAIT = 600
SEEDANCE_POLL_INTERVAL = 5.0
...
def _edit_via_seedance(
    *,
    ...
    """SeaDance GPT-Image-2 image-to-image / upscale adapter."""
    api_key = _get_provider_api_key("seedance")
    if not api_key:
        return {"success": False, "error": "未配置 SEEDANCE_API_KEY，无法使用 SeaDance 图生图/超分"}
    ...
    # 1K 在测试中最稳定；后续可按 upscale_factor 映射到更高分辨率
    resolution = "1K"
    payload: Dict[str, Any] = {
        "model": model_id,
        "image": source_image_url,
        "prompt": prompt or "enhance details, high definition",
    ...
    try:
        ...
        return {"success": False, "error": f"SeaDance 提交失败: {exc}"}

    task_id = data.get("taskId")
    if not task_id:
        return {"success": False, "error": f"SeaDance 未返回 taskId: {data}"}

    poll_url = f"{base_url}/task/{task_id}"
    deadline = time.monotonic() + SEEDANCE_MAX_WAIT
    while time.monotonic() < deadline:
        try:
            ...
            return {"success": False, "error": f"SeaDance 轮询失败: {exc}"}

        state = poll_data.get("state")
        if state == "success":
            result_urls = poll_data.get("video_urls") or []
            if not result_urls:
                return {"success": False, "error": "SeaDance 未返回结果 URL"}
            local_paths: list[str] = []
            for idx, result_url in enumerate(result_urls):
                suffix = f"_{idx + 1}" if len(result_urls) > 1 else ""
                local_path = download_file(result_url, f"{suffix}.png", "image")
                if local_path:
                    local_paths.append(local_path)
            if not local_paths:
                return {"success": False, "error": "下载 SeaDance 结果失败"}
            return {
                "success": True,
                "url": result_urls[0],
                "urls": result_urls,
            ...
            return {
                "success": False,
                "error": (
                    f"SeaDance 任务失败 ({poll_data.get('failCode', 'unknown')}): "
                    f"{poll_data.get('failMsg', 'no message')}"
                ),
            }

        time.sleep(SEEDANCE_POLL_INTERVAL)

    return {"success": False, "error": "SeaDance 任务轮询超时"}
...
def run_image_edit(
    *,
    ...
    api_type = selected.get("api_type", "dashscope_wanx_image_edit")

    logger.info(
        "[ImageEdit] mode=%s api_type=%s model=%s source=%s prompt=%s n=%s refs=%s intent=%s wm=%s",
        mode,
        api_type,
        model_id,
        source_image_url[:80],
    ...
    if api_type == "seedance_image_edit":
        if mode not in ("upscale", "instruction"):
            return {
                "success": False,
                "error": f"SeaDance 图生图当前仅支持超分/指令编辑模式，不支持 {mode}",
                "mode": mode,
                "model": model_name,
            }
        try:
            ...
            logger.warning("[ImageEdit] SeaDance failed: %s", exc, exc_info=True)
            result = {"success": False, "error": str(exc)}
        provider_name = "seedance"

    elif mode == "watermark" and wm_mode == "text":
...
@tool
def seedance_enhance_image(
    source_image_url: str,
    prompt: str = "",
    resolution: str = "1K",
) -> str:
    """使用 SeaDance GPT-Image-2 对图片进行高清增强处理，提升细节、锐度与分辨率。

    Args:
        source_image_url: 要增强的图片 URL（支持 http/https 或 /api/media/ 路径）
        prompt: 增强提示词，如"enhance details, ultra sharp, 4K, high definition"；留空则使用默认提示
        resolution: 输出分辨率，可选 1K（默认）、2K、4K

    Returns:
        增强后的图片 URL 和本地路径信息
    """
    if not source_image_url:
        return "❌ 请提供要增强的图片 URL"

    api_key = _get_provider_api_key("seedance")
    if not api_key:
        return "❌ 未配置 SEEDANCE_API_KEY，无法使用 SeaDance 高清增强"
    ...
    logger.info(
        "[SeaDance/Enhance] submit: resolution=%s prompt=%s url=%s",
        resolution,
        effective_prompt[:60],
        source_image_url[:80],
    )
    ...
    try:
        ...
        return f"❌ SeaDance 提交失败: {exc}"

    task_id = submit_data.get("taskId")
    if not task_id:
        return f"❌ SeaDance 未返回 taskId: {submit_data}"

    logger.info("[SeaDance/Enhance] task_id=%s", task_id)
    ...
    while time.monotonic() < deadline:
        try:
            ...
            return f"❌ SeaDance 轮询失败: {exc}"

        state = poll_data.get("state")
        if state == "success":
            result_urls: List[str] = (
                poll_data.get("image_urls")
                or poll_data.get("video_urls")
                or poll_data.get("urls")
                or []
            )
            if not result_urls:
                return f"❌ SeaDance 增强完成但未返回结果 URL: {poll_data}"

            output_url = result_urls[0]
            local_path = download_file(output_url, ".png", "image")
            logger.info("[SeaDance/Enhance] done: url=%s local=%s", output_url, local_path)

            media_url = (
                f"/api/media/{os.path.basename(local_path)}"
                if local_path
                else output_url
            )
            return (
                f"✅ 图片高清增强完成（分辨率: {resolution}）\n\n"
                f"![增强结果]({media_url})\n\n"
                f"**图片 URL:** {media_url}\n"
                f"**原始 URL:** {output_url}"
            )
            ...
            return (
                f"❌ SeaDance 增强失败 ({poll_data.get('failCode', 'unknown')}): "
                f"{poll_data.get('failMsg', 'no message')}"
            )

        time.sleep(SEEDANCE_POLL_INTERVAL)

    return "❌ SeaDance 高清增强任务轮询超时（超过 10 分钟）"
