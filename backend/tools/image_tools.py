...
def generate_image(prompt: str) -> str:
    ...
    try:
        ...
        if api_type == "openrouter":
        ...
        elif api_type == "seedance_text2image":
            api_key = _get_provider_api_key("seedance")
            if not api_key:
                return "❌ 未配置 SEEDANCE_API_KEY，无法使用 SeaDance 文生图"
            ...
            payload: Dict[str, Any] = {
                "model": model_id or "gpt-image-2-text-to-image",
                "prompt": prompt,
                "aspect_ratio": "1:1",
                "resolution": "1K",
            }
            ...
            logger.info(f"[SeaDance/T2I] task created: {task_id}, model={model_id}")
            ...
            while _time.monotonic() < deadline:
                ...
                if state == "success":
                    ...
                    if not result_urls:
                        return f"❌ SeaDance 未返回结果 URL: {poll_data}"
                    url = result_urls[0]
                    local_path = download_file(url, ".png", "image")
                    model_used = model_id
                    logger.info(f"[SeaDance/T2I] image ready: {url}")
                    break
                elif state == "fail":
                    return (
