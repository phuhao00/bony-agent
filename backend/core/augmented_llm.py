...
# Agents whose tools may perform long-running media generation (video, image edit,
# etc.).  The default 75s timeout is too short for these workflows, so we give
# them up to 10 minutes to finish.
_LONG_RUNNING_AGENT_IDS = frozenset({
    "media_agent",
    "video_editor_agent",
    "opencut_agent",
    "long_video_agent",
...
def _agent_timeout(agent_id: Optional[str], default: float = 75.0) -> float:
    return 600.0 if agent_id in _LONG_RUNNING_AGENT_IDS else default
...
def build_augmented_executor(
    *,
    ...
    async def run_augmented_agent(input_state):
        ...
        timeout = _agent_timeout(agent_id)
        result = await asyncio.wait_for(agent.ainvoke(input_state), timeout=timeout)
