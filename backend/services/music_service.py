...
def submit_recipe(
    ...
    *,
    ...
    """创建任务并立即返回 task_id，不执行耗时的 LLM 调用。"""
    recipe = get_recipe(recipe_id)
    if not recipe:
    ...
    task_manager.update_task(task_id, status="running", progress=5, message=f"已提交：{recipe.name}")
    return task_id
...
def execute_recipe(
    ...
    """在后台线程中执行耗时的 LLM 分析，更新 task_manager 状态。"""
    recipe = get_recipe(recipe_id)
    if not recipe:
        task_manager.update_task(task_id, status="failed", progress=100, message=f"Unknown recipe: {recipe_id}")
        return
    ...
    try:
        ...
        task_manager.update_task(
            task_id, status="failed", progress=100, message=str(exc), result={"error": str(exc)}
        )
        return
...
def start_recipe(
    ...
    *,
    ...
    """兼容旧调用，同步执行（内部调用 submit + execute）。"""
    task_id = submit_recipe(recipe_id, params, trace_id=trace_id)
    execute_recipe(task_id, recipe_id, params)
    ...
    return {
        ...
        "error": task.get("message") if task.get("status") == "failed" else None,
    }
